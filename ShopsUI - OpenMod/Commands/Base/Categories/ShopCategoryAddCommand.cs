...
{
    public abstract class ShopCategoryAddCommand<TCategoryEditor> : ShopCategoryCommandBase
        where TCategoryEditor : ICategoryEditor
    {
        private readonly TCategoryEditor _categoryEditor;

        protected ShopCategoryAddCommand(IServiceProvider serviceProvider) : base(serviceProvider)
        {
            _categoryEditor = serviceProvider.GetRequiredService<TCategoryEditor>();
        }

        protected abstract bool CheckValidId(ushort id);
        ...
        protected override async UniTask OnExecuteAsync()
        {
            ...
            {
                if (ushort.TryParse(idRangeStr, out var id) && CheckValidId(id))
                {
            ...
            var result = await _categoryEditor.AddShopsToCategory(categoryName, shopIds) ??
            ...
            await PrintAsync(StringLocalizer[GetSuccessLocalizationKey(),
                new {ShopsAdded = result, CategoryName = categoryName}]);
        }
    }
}
